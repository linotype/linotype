...
const srcGenerator = (item, format = 'jpg', fit = 'contain', quality = 90, xN = 1 ) => {
  return encodeURI( `${config.linotype.backend_url}/assets/${item.source.id}?format=${format}&quality=${quality}&transforms=[["resize",{"fit":"${fit}","background":"rgba(0,0,0,0)","width": ${item?.size[0]*xN || null},"height": ${item?.size[1]*xN || null}}]]` )
}
